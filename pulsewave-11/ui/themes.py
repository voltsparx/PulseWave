...
class Theme:
    ...
    paused_icon: str = "⏸"
    stopped_icon: str = "■"
    queue_active_marker: str = "▶"
    queue_idle_marker: str = "·"
    accent_divider: str = "•"
    ...
    def load_theme(self, name: str) -> Theme:
        ...
        if self.theme_dir:
            ...
            if candidate.exists():
                ...
                base = BUILTIN_THEMES.get(key, BUILTIN_THEMES["default"])
                return Theme(
                    name=key,
                    border_style=parsed.get("border_style", base.border_style),
                    highlight_style=parsed.get("highlight_style", base.highlight_style),
                    progress_style=parsed.get("progress_style", base.progress_style),
                    visualizer_style=parsed.get("visualizer_style", base.visualizer_style),
                    accent_style=parsed.get("accent_style", base.accent_style),
                    muted_style=parsed.get("muted_style", base.muted_style),
                    error_style=parsed.get("error_style", base.error_style),
                    border_type=parsed.get("border_type", base.border_type),
                    progress_fill=_single_char(parsed.get("progress_fill"), fallback=base.progress_fill),
                    progress_empty=_single_char(parsed.get("progress_empty"), fallback=base.progress_empty),
                    visualizer_levels=parsed.get("visualizer_levels", base.visualizer_levels),
                    header_left=_single_char(parsed.get("header_left"), fallback=base.header_left),
                    header_right=_single_char(parsed.get("header_right"), fallback=base.header_right),
                    playing_icon=_single_char(parsed.get("playing_icon"), fallback=base.playing_icon),
                    paused_icon=_single_char(parsed.get("paused_icon"), fallback=base.paused_icon),
                    stopped_icon=_single_char(parsed.get("stopped_icon"), fallback=base.stopped_icon),
                    queue_active_marker=_single_char(parsed.get("queue_active_marker"), fallback=base.queue_active_marker),
                    queue_idle_marker=_single_char(parsed.get("queue_idle_marker"), fallback=base.queue_idle_marker),
                    accent_divider=_single_char(parsed.get("accent_divider"), fallback=base.accent_divider),
                )
